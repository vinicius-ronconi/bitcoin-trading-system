...
class TrailingOrdersTestCase(TestCase):
    ...
    def test_it_places_a_buy_order(self):
        self.system.set_state(states.TrackingToBuyState(self.system))
        self._set_last_quote(self.system.buy_price)
        self.system.run()
        self._assert_results(buy_call_count=1, expected_state=states.PendingToBuyState)

        self.system.set_state(states.TrackingToBuyState(self.system))
        self._set_last_quote(self.system.buy_price + 0.01)
        self.system.run()
        self._assert_results(buy_call_count=2, expected_state=states.PendingToBuyState)

    def test_it_does_not_place_a_buy_order_besides_is_tracking(self):
        self.system.set_state(states.TrackingToBuyState(self.system))
        self._set_last_quote(self.system.buy_price - 0.01)
        self.system.run()
        self._assert_results(buy_call_count=0, expected_state=states.TrackingToBuyState)

    def test_it_starts_tracking(self):
        self.system.set_state(states.WaitingToBuyState(self.system))
        ...
        self.system.set_state(states.WaitingToBuyState(self.system))
        last_quote = get_rounded_decimal_value(self.system.setup.start_value / 2)
    ...
    def test_it_does_not_start_tracking(self):
        ...
        self._set_last_quote(self.system.setup.start_value + 0.01)
        self.system.run()
        self._assert_results(buy_call_count=0, expected_state=states.WaitingToBuyState)

    def test_it_updates_setup_values(self):
        self.system.set_state(states.WaitingToBuyState(self.system))
        ...
        self.system.update_setup(
            beans.TrailingOrderSetup(
                next_operation=consts.OrderSide.BUY,
                start_value=self.system.current_ticker.lowest_value / 2,
        ...
        last_quote = self.system.setup.start_value + 1.0
        self._set_last_quote(last_quote)
        self.system.run()
        self._assert_results(buy_call_count=0, expected_state=states.WaitingToBuyState)
        self.assertEqual(self.system.setup.start_value, self.START_VALUE)
        self.assertEqual(
            self.system.setup.stop_value,
            self.system.setup.stop_value * (self.system.current_ticker.lowest_value / self.system.setup.start_value)
        )

        self.system.update_setup(self.INITIAL_SETUP)
        self.system.set_state(states.TrackingToBuyState(self.system))
        last_quote = get_rounded_decimal_value(self.system.setup.start_value / 2)
        self._set_last_quote(last_quote)
        self.system.run()
    ...
    def test_it_goes_to_selling_mode(self):
        self.system.set_state(states.PendingToBuyState(self.system))
        self.system.get_pending_orders.return_value = []
        self.system.run()
        self._assert_results(buy_call_count=0, expected_state=states.WaitingToSellState)

    def test_it_goes_back_to_wait_to_buy(self):
        self.system.set_state(states.PendingToBuyState(self.system))
        self.system.get_pending_orders.return_value = [
            PlacedOrder(
    ...
    def _assert_results(self, buy_call_count, expected_state):
        ...
        self.assertEqual(self.system.client.orders.sell_bitcoins_with_market_order.call_count, 0)
